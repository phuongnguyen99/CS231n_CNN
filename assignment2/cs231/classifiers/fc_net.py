...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(hidden_num):
            if self.normalization == "batchnorm":
                scores, cache = affine_batch_relu_forward(scores,
                                                      self.params['W%d' % (i + 1)],
                                                      self.params['b%d' % (i + 1)],
        ...
        while i >= 0:
            ...
            if self.normalization == "batchnorm":
                dout, grads['W%d' % (i + 1)], grads['b%d' % (i + 1)], grads['gamma%d' % (i + 1)], grads['beta%d' % (i + 1)] = affine_batch_relu_backward(dout, cache_history.pop())
            else:
